...
subroutine initfields_wallM(decompC, decompE, inputfile, mesh, fieldsC, fieldsE)
    ...
    namelist /PROBLEM_INPUT/ Lx, Ly, Lz, Tref, Tsurf0, dTsurf_dt, z0init, frameAngle, init_T_profile
    ...
    case(0)
       T = 0.01d0*(ztmp - 100.d0) + Tsurf0
       where(ztmp < 100.d0)
          T = Tsurf0
       end where
       T = T + 0.0001d0*ztmp

    ! from Gadde and Stevens 2021
    case(1)
       do k = 1, decompC%xsz(3)
          if (ztmp(1,1,k)<1000.d0) then
             T(:,:,k) = 286
          elseif (ztmp(1,1,k)>=1000.d0 .and. ztmp(1,1,k)<1150.d0) then
             T(:,:,k) = 286 + (ztmp(1,1,k)-1000.d0) * 6.d0 / 150.d0
          elseif (ztmp(1,1,k)>=1150.d0) then
             T(:,:,k) = Tsurf0 + (1150.d0 - 1000.d0) * 6.d0 / 150.d0 + (ztmp(1,1,k)-1150.d0) * 5.d0 / 1000.d0
          end if
       end do
       
    ! inversion layer of strength 3 K/km
    case(2)
    ...
    ! inversion layer of strength 5 K/km
    case(3)
       T = 0.005d0*(ztmp - 100.d0) + Tsurf0
       where(ztmp < 100.d0)
          T = Tsurf0
       end where
       T = T + 0.0001d0*ztmp

    ! inversion layer of strength 8 K/km
    case(4)
       T = 0.008d0*(ztmp - 100.d0) + Tsurf0
       where(ztmp < 100.d0)
          T = Tsurf0
       end where
       T = T + 0.0001d0*ztmp
...
subroutine setInhomogeneousNeumannBC_Temp(inputfile, wTh_surf)
    ...
    namelist /PROBLEM_INPUT/ Lx, Ly, Lz, Tref, Tsurf0, dTsurf_dt, z0init, init_T_profile
...
subroutine meshgen_wallM(decomp, dx, dy, dz, mesh, inputfile)
    ...
    integer :: i,j,k, ioUnit, init_T_profile
    character(len=*),                intent(in)    :: inputfile
