...
subroutine initfields_wallM(decompC, decompE, inputfile, mesh, fieldsC, fieldsE)
    ...
    case(1)
        do k = 1, decompC%xsz(3)
            if (ztmp(1,1,k)<400.d0) then
                T(:,:,k) = Tsurf0
            elseif (ztmp(1,1,k)>=400.d0 .and. ztmp(1,1,k)<500.d0) then
                T(:,:,k) = Tsurf0 + (ztmp(1,1,k)-400.d0) * 6.d0 / 100.d0
            elseif (ztmp(1,1,k)>=500.d0) then
                T(:,:,k) = Tsurf0 + (500.d0 - 400.d0) * 6.d0 / 100.d0 + (ztmp(1,1,k)-500.d0) * 1.d0 / 50.d0
            end if
        end do
    
    ! Gabls initial probile for the stable case
    case(2)
        T = 0.003d0*(ztmp - 200.d0) + Tsurf0
        where(ztmp < 200.d0)
            T = Tsurf0
        end where
        T = T + 0.0001d0*ztmp
