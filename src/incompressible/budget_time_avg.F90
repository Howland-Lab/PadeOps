module budgets_time_avg_mod
   ...
   type :: budgets_time_avg
	...
	complex(rkind), dimension(:,:,:), allocatable, public :: HITforcing_x, HITforcing_y, HITforcing_z 
        type(igrid), pointer, public :: igrid_sim 
        
        real(rkind), dimension(:,:,:,:), allocatable, public :: budget_0, budget_1, budget_2, budget_3
        integer :: counter
...
contains 
	...
	! 8. HIT forcing-velocity correlation (energy injection)
	call this%igrid_sim%spectC%ifft(this%HITforcing_x, this%igrid_sim%rbuffxC(:,:,:,1))
	this%budget_3(:,:,:,8) = this%budget_3(:,:,:,8) + this%igrid_sim%rbuffxC(:,:,:,1)*this%igrid_sim%u

	call this%igrid_sim%spectC%ifft(this%HITforcing_y, this%igrid_sim%rbuffxC(:,:,:,1))
	this%budget_3(:,:,:,8) = this%budget_3(:,:,:,8) + this%igrid_sim%rbuffxC(:,:,:,1)*this%igrid_sim%v

	call this%igrid_sim%spectE%ifft(this%HITforcing_z, this%igrid_sim%rbuffxE(:,:,:,1))
	call this%interp_Edge2Cell(this%igrid_sim%rbuffxE(:,:,:,1), this%igrid_sim%rbuffxC(:,:,:,1))
	this%budget_3(:,:,:,8) = this%budget_3(:,:,:,8) + this%igrid_sim%rbuffxC(:,:,:,1)*this%igrid_sim%wC
    ...
    if(this%do_budgets) then
        ...
        if (allocated(this%HITforcing_x)) deallocate(this%HITforcing_x, this%HITforcing_y, this%HITforcing_z)
        deallocate(this%budget_0, this%budget_1, this%budget_2, this%budget_3)
