module spectralMod
    ...
    external :: DFFTW_PLAN_MANY_DFT_R2C, DFFTW_PLAN_MANY_DFT_C2R, &
    ...
    contains
        ...
        procedure           :: bandpassFilter_and_PhaseShift_z
...
! Does to bandpassFilter_and_PhaseShift as a function of z
    subroutine bandpassFilter_and_PhaseShift_z(this, uhat, uFilt, xshift, yshift)
        use constants, only: imi

        class(spectral),  intent(inout)         :: this
        complex(rkind), dimension(this%spectdecomp%ysz(1),this%spectdecomp%ysz(2), this%spectdecomp%ysz(3)), intent(in) :: uhat
        real(rkind)   , dimension(this%physdecomp%xsz(1),this%physdecomp%xsz(2), this%physdecomp%xsz(3)), intent(out) :: uFilt
        real(rkind), dimension(this%spectdecomp%zsz(3)), intent(in) :: xshift
        real(rkind), dimension(this%spectdecomp%zsz(3)), intent(in), optional :: yshift
        complex(rkind) :: shiftfact
        integer :: i, j, k

        call transpose_y_to_z(uhat, this%cbuffz_bp, this%spectdecomp)
        
        do k = 1,size(this%cbuffz_bp,3)  ! this is in physical space
            do j = 1,size(this%cbuffz_bp,2)  ! now in spectral space
                do i = 1,size(this%cbuffz_bp,1)  ! in spectral space
                    ! shift in x, y
                    shiftfact = exp(-imi * this%k1inZ(i) * xshift(k))
                    if (present(yshift)) then
                        shiftfact = shiftfact * exp(-imi * this%k2inZ(j) * yshift(k))
                    endif 

                    this%cbuffz_bp(i,j,k) = this%G_bandpass(i,j,k)*this%cbuffz_bp(i,j,k)*shiftfact
                end do
            end do
        end do
