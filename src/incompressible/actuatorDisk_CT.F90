module actuatorDisk_CTMod
    ...
    private
    public :: actuatorDisk_ct
     
    type :: actuatorDisk_ct
        ! Quick change to Filtered ADM (Shapiro et al. (2022)) to use C_T instead of C_T'
        ! Kirby Heck 09/17/2023
        ...
        real(rkind), dimension(:), allocatable :: powerTime, uTime, vTime, thrustTime
...
subroutine init(this, inputDir, ActuatorDisk_ID, xG, yG, zG)
    ...
    call message(1, "Initializing Actuator Disk (ADM Type=6) number", ActuatorDisk_ID)
    call tic()
...
!        write(*,*) "Only one allocated? YES/NO"
        ...
        allocate(this%thrustTime(1000000))
    end if
...
subroutine get_RHS(this, u, v, w, rhsxvals, rhsyvals, rhszvals, yaw, theta)
    ...
    ! Mean speed at the turbine, no correction factor M
    this%ut = p_sum(this%scalarSource*this%speed)*this%dV
    vface = p_sum(this%scalarSource*(u*tau(1,1) + v*tau(2,1) + w*tau(3,1)))*this%dV
    usp_sq = one  ! (this%ut)**2  ! modified here
    ...
    !        if((this%Am_I_Split .and. this%myComm_nrank==0) .or. (.not. this%Am_I_Split)) then
        if (usp_sq /= 0.d0) then
            ...
            this%thrustTime(this%tInd) = force
