...
contains
    ...
    subroutine init(this, inputfile, igrid_sim, amplit_inflow, freq_inflow)
        ...
        ! TODO: Make these optional again once I have it working!
        real(rkind), intent(in) :: amplit_inflow, freq_inflow ! if inflow is surging, these must be provided
    ...
    subroutine doBudgets(this, phase_inflow, forceDump)
        ...
        if (this%do_budgets)  then
            ...
            ! elseif (present(phase_inflow)) then ! if not a moving turbine, then the inflow must be surging
            sim_curr_phase = phase_inflow
            ! else
            !     call GracefulExit("Phase-averaged budgets turned on, but turbine isn't moving and phase_inflow not provided to doBudgets.", 101)
            ! endif
